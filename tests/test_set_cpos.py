...
def test_set_cpos_plus_x_places_camera_on_positive_x(renderer):
    renderer.set_cpos('+x')
    cam = renderer._plotter.camera
    assert cam.position[0] > cam.focal_point[0]
    assert abs(cam.position[1] - cam.focal_point[1]) < 1e-6
    assert abs(cam.position[2] - cam.focal_point[2]) < 1e-6
    assert abs(cam.up[2] - 1.0) < 1e-6


def test_set_cpos_bare_x_aliases_plus_x(renderer):
    renderer.set_cpos('x')
    cam = renderer._plotter.camera
    assert cam.position[0] > cam.focal_point[0]
    assert abs(cam.up[2] - 1.0) < 1e-6
...
def test_set_cpos_minus_x_places_camera_on_negative_x(renderer):
    renderer.set_cpos('-x')
    cam = renderer._plotter.camera
    assert cam.position[0] < cam.focal_point[0]
    assert abs(cam.up[2] - 1.0) < 1e-6
...
def test_set_cpos_plus_z_places_camera_on_positive_z(renderer):
    renderer.set_cpos('+z')
    cam = renderer._plotter.camera
    assert cam.position[2] > cam.focal_point[2]
    assert abs(cam.up[1] - 1.0) < 1e-6
